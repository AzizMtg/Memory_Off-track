...
if (event.motion.y<=328 && event.motion.y>=130 && (event.motion.x<=791 && event.motion.x>=340)) //il bouton play il 2 sout wa animation 
     {afficher_imageloadb( screen , IMAGE_loadb) ; 
      ...
      //event.type == SDL_MOUSEBUTTONUP :un clic de la souris
      case SDL_MOUSEBUTTONDOWN : 
       ...
       boucle=0  ;  
 /////////////////////////extra /////////////////////////////////////////     
               if(event.motion.y<=320 && event.motion.y>=280 && (event.motion.x<=992 && event.motion.x>=892 )) // he8i taba3 il bouton extra injarbou feha 
              {  while(boucle3)  
