...
int testi = 0  , k=0 ; // itesti il coueur 
...
int j=0;
// il vie 
int p_min_x =IMAGE_main.pos_Screen.x ;
int p_max_x =IMAGE_main.pos_Screen.x  + 100;
int p_min_y = IMAGE_main.pos_Screen.y;
int p_max_y = IMAGE_main.pos_Screen.y + 100;

int e_min_x = E.pos_actuelle.x;
int e_max_x = E.pos_actuelle.x + 100;
int e_min_y = E.pos_actuelle.y;
int e_max_y = E.pos_actuelle.y + 100;

int e2_min_x = E2.pos_actuelle.x;
int e2_max_x = E2.pos_actuelle.x + 100;
int e2_min_y = E2.pos_actuelle.y;
int e2_max_y = E2.pos_actuelle.y + 100;


if((e_min_x < p_max_x) &&( e_max_x > p_min_x) && (e_min_y < p_max_y )&&( e_max_y > p_min_y)) 
{
      i += 200;
      initialiser_coeur(&coeur, i);
    }

if((e2_min_x < p_max_x) &&( e2_max_x > p_min_x) && (e2_min_y < p_max_y )&&( e2_max_y > p_min_y)) 
{
      i += 200;
      initialiser_coeur(&coeur, i);
    }
...
/// il score 
if (IMAGE_main.pos_Screen.x ==200&&IMAGE_main.pos_Screen.x <=450&&IMAGE_main.pos_Screen.x >=350) 
{if (i2 <57 ) 
{i2=i2+1 ;
initialiser_texte1(&txte1,i3,i2) ;}
else
...
printf("%d ",B.camera_pos.x);
   if (B.camera_pos.x==2000)
 {level2(val);}  
...
//level2
	 ...
	 if (event.key.keysym.sym == SDLK_h) // touch deceleration 
         ...
         //aziz
          ...
          if (event.key.keysym.sym == SDLK_p && B.camera_pos.x>1248 && B.camera_pos.x<1878) // touch acceleration  
              { 
...
liberer_texte_score(texte_score);
liberer_imageb(feu) ; 
...
//aziz
	e.screen=screen;
        initialiser_image_level2(&IMG);
	initialiser_imageBOUTON1_level2(&IMAGE_BTN1) ; 
	initialiser_imageBOUTON2_level2(&IMAGE_BTN2) ; 
    initialiser_audioe(music2);
e.f = fopen("enigme.txt", "r");
 if (e.f==NULL)
	{
...
afficherEnnemibig(E3,screen);
animerEnemibig(&E3);
movebig(&E3, B.camera_pos);

afficherEnnemibig(E4,screen);
animerEnemibig(&E4);
movebig(&E4, B.camera_pos);

afficherEnnemibig(E5,screen);
animerEnemibig(&E5);
movebig(&E5, B.camera_pos);
...
int e3_min_x = E3.pos_actuelle.x;
int e3_max_x = E3.pos_actuelle.x + 100;
int e3_min_y = E3.pos_actuelle.y;
int e3_max_y = E3.pos_actuelle.y + 100;

int e4_min_x = E4.pos_actuelle.x;
int e4_max_x = E4.pos_actuelle.x + 100;
int e4_min_y = E4.pos_actuelle.y;
int e4_max_y = E4.pos_actuelle.y + 100;

int e5_min_x = E5.pos_actuelle.x;
int e5_max_x = E5.pos_actuelle.x + 100;
int e5_min_y = E5.pos_actuelle.y;
int e5_max_y = E5.pos_actuelle.y + 100;


if((e3_min_x < p_max_x) &&( e3_max_x > p_min_x) && (e3_min_y < p_max_y )&&( e3_max_y > p_min_y)) 
{
      i += 200;
      initialiser_coeur(&coeur, i);
    }

if((e4_min_x < p_max_x) &&( e4_max_x > p_min_x) && (e4_min_y < p_max_y )&&( e4_max_y > p_min_y)) 
{
      i += 200;
      initialiser_coeur(&coeur, i);
    }

if((e5_min_x < p_max_x) &&( e5_max_x > p_min_x) && (e5_min_y < p_max_y )&&( e5_max_y > p_min_y)) 
{
      i += 200;
      initialiser_coeur(&coeur, i);
    }
...
bouclenigme=1;
		while( bouclenigme ){
					 afficher_imagestory(screen, IMG);
...
bouclenigme = 0;
	...
	afficher_imagestory(screen,IMAGE_BTN1 ) ;
	SDL_Flip(screen);
}
...
bouclenigme = 0;
	...
	afficher_imagestory(screen,IMAGE_BTN2 ) ;
...
liberer_ennemibig(E3);
liberer_ennemibig(E4);
liberer_ennemibig(E5);
...
void initPerso2(personne *p , int v   )  // 3awed billahi a3mil copier lil fonction iniperson wa fonction init zidt fihom 7ajet 
{
switch (v ) 
{
case 1 : 
...
if (p->acceleration == 0 ) 
{
  if (p->direction==1 )  
     {p->vitesse=0;
       ...
       if ( p->pos_Screen.x<=200&& d==1)    /// how mil hna yit9sam 3ala 1 he8a il joueur 2 [296..500]   il machya yousil 7ata il 500 joueur 1 
                  {  p->pos_Screen.x=p->pos_Screen.x+5 ;      
                     p->pos_Screen.y=p->pos_Screen.y ;
                  } 
       if ( p->pos_Screen.x<=750&& d==2) // [ 0..210 ] joueur 2 il machya 
                  {  p->pos_Screen.x=p->pos_Screen.x+5 ;      
                     p->pos_Screen.y=p->pos_Screen.y ;
                  } 
     ...
     { 
      ...
      if (p->pos_Sprite.x <=520)
         ...
         if ( p->pos_Screen.x>=0&&d==1)   // he8i il raj3a joueur 1 
                   p->pos_Screen.x=p->pos_Screen.x-5; 

           if ( p->pos_Screen.x>600&&d==2)   // joueur 2 il raj3a  
                   p->pos_Screen.x=p->pos_Screen.x-5; 
    }
...
void saut2( personne *p,int dt, int posinit)  // saut normale
{     
if (p->up == 1  ) 
{
  if (p->direction==1 || p->direction== 2  )  
...
void init2(personne * p, int numperso)   //  initialisation de la perosnnage principale
{
p->url="multiplayer.png" ; 
p->sprite=IMG_Load(p->url) ; 
if (p->sprite == NULL) 
{
printf("unable to load background image %s \n",SDL_GetError()) ;
...
b->pos_img_ecran1.x=0; 
b->pos_img_ecran1.y=0; 
b->pos_img_ecran1.h=0; 
b->pos_img_ecran1.w=SCREEN_W/2; 

b->camera_pos1.x=0; 
b->camera_pos1.y=(700-600)/2; 
b->camera_pos1.h=SCREEN_H ; 
b->camera_pos1.w=SCREEN_W/2 ;

b->pos_img_ecran2.x=SCREEN_W/2; 
b->pos_img_ecran2.y=0; 
b->pos_img_ecran2.h=0; 
b->pos_img_ecran2.w=SCREEN_W/2; 


b->camera_pos2.x=0; 
b->camera_pos2.y=(700-600)/2; 
b->camera_pos2.h=SCREEN_H ; 
b->camera_pos2.w=SCREEN_W/2 ;
...
{ 
SDL_BlitSurface(b.img ,&b.camera_pos1 , screen , &b.pos_img_ecran1 ) ; 
SDL_BlitSurface(b.img ,&b.camera_pos2 , screen , &b.pos_img_ecran2 ) ; 
}
...
void scrollingone(backgroundtwo *b, int direction_one,int pas) {
    switch (direction_one) {
        case 0:
            b->camera_pos1.x += pas;
            break;
        case 1:
            b->camera_pos1.x = b->camera_pos1.x - pas;
            break;
        case 3:
            b->camera_pos1.y = b->camera_pos1.y + pas;
            break;
        case 2:
            b->camera_pos1.y = b->camera_pos1.y - pas;
            break;
    ...
    if (b->camera_pos1.x + b->camera_pos1.w > 3200) {
        b->camera_pos1.x = 3200 - b->camera_pos1.w;
    }
    ...
    if (b->camera_pos1.y + b->camera_pos1.h > 700) {
        b->camera_pos1.y =700 - b->camera_pos1.h;
    }
}
...
void scrollingtwo(backgroundtwo *b, int direction_two,int pas) {
    switch (direction_two) {
        case 0:
            ...
            break;
        case 1:
            b->camera_pos2.x = b->camera_pos2.x - pas;
            break;
        case 3:
            b->camera_pos2.y = b->camera_pos2.y + pas;
            break;
        case 2:
            b->camera_pos2.y = b->camera_pos2.y - pas;
            break;
    ...
    if (b->camera_pos2.x < 15) {
        b->camera_pos2.x = 15;
    }
    if (b->camera_pos2.x + b->camera_pos2.w > 3200) {
        b->camera_pos2.x = 3200 - b->camera_pos2.w;
    }
    ...
    if (b->camera_pos2.y + b->camera_pos2.h > 700) {
        b->camera_pos2.y =700 - b->camera_pos2.h;
    }
}
...
SDL_Surface *screen ; 
personne IMAGE_main ,coeur ,lost , IMAGE_JOUEUR2 ; 
image1 IMAGE ,feu ; 
SDL_Event   event ;   
int boucle =1 ;
int test = 0 ; 
...
initialiser_feu2(&feu)  ;
initPerso2(&IMAGE_main ,1)  ;
init2(&IMAGE_JOUEUR2, 1)  ;

IMAGE_main.vie = 9 ; IMAGE_main.score = -1 ;  
initialiser_coeur2(&coeur ,i ,&IMAGE_main.vie) ; 
initialiser_lost2(&lost ); 


initialiser_texte2(&txte,i3,i2,& IMAGE_main.score) ; 
...
if (IMAGE_main.pos_Screen.x ==150&&IMAGE_main.pos_Screen.y <=450&&IMAGE_main.pos_Screen.y >=350) 
{i= i+25 ;
initialiser_coeur2(&coeur ,i ,&IMAGE_JOUEUR2.vie) ;
...
afficher_feu2(screen ,feu ) ;
afficherPerso2(IMAGE_main ,screen) ;	

afficherPerso2(IMAGE_JOUEUR2 ,screen) ;		

afficherPerso2(coeur ,screen) ;  
if (i==200)
   {while (boucle2)
       {afficherPerso2(lost ,screen) ;
        initialiser_audiobrefj2(mus2) ; 
        while(SDL_PollEvent(&event)) 
          { switch(event.type)
              {case  SDL_QUIT:  
...
afficher_texte2(screen,txte) ;

   if (IMAGE_main.pos_Screen.x==200)
 {animerPerso2 (&IMAGE_main)  ;}  
         ...
         if (event.key.keysym.sym==SDLK_DOWN)  
           ...
           if (event.key.keysym.sym==SDLK_UP) 
             ...
             if (event.key.keysym.sym==SDLK_ESCAPE)   
                {IMAGE_main.up=5; 
                 drouj2 ( &IMAGE_main , IMAGE_main.pos_Screen.x ,  IMAGE_main.pos_Screen.y)  ;
                  //// khalil 
                    direction_one  = 2;
                        con=1; }  
...
/////////////////////////////////////
if (event.key.keysym.sym == SDLK_t)  
              { IMAGE_JOUEUR2.d  =-1 ; IMAGE_JOUEUR2.acceleration=0; 
                 indicate2 = 1 ;
                initialiser_audiobrefj3(mus3) ;
               }
               
           if (event.key.keysym.sym == SDLK_d)
              {  IMAGE_JOUEUR2.direction = 1 ; IMAGE_JOUEUR2.acceleration=0;
                 indicate2 = 1 ;
                initialiser_audiobrefj1(mus1) ;
          ...
          if (event.key.keysym.sym==SDLK_q)  
                {IMAGE_JOUEUR2.direction = 0;  IMAGE_JOUEUR2.acceleration=0;
                 indicate2 = 1 ; 
                 initialiser_audiobrefj1(mus1) ;
                  ////  khalil 
                   direction_two = 1;
          ...
          if (event.key.keysym.sym == SDLK_e)  
              { IMAGE_JOUEUR2.acceleration+=0.005;}

           if (event.key.keysym.sym == SDLK_r)  
              {IMAGE_JOUEUR2.acceleration-=0.11;}

         
         if (event.key.keysym.sym==SDLK_s)  
               {  if ( IMAGE_JOUEUR2.pos_Screen.y<=500) 
                    {IMAGE_JOUEUR2.pos_Screen.y=IMAGE_JOUEUR2.pos_Screen.y+20 ;  }
                   //// khalil 
                     direction_two = 3;
                        con_two=1;
               }
           ...
           if (event.key.keysym.sym==SDLK_x) 
                {IMAGE_JOUEUR2.up=1; pos1 = IMAGE_JOUEUR2.pos_Screen.y ; indicates2=1 ; 
                   IMAGE_JOUEUR2.pos_Screen.y =pos1 ;}  
 
             if (event.key.keysym.sym==SDLK_z)   
                {IMAGE_JOUEUR2.up=5; 
                 drouj2 ( &IMAGE_JOUEUR2 , IMAGE_JOUEUR2.pos_Screen.x ,  IMAGE_JOUEUR2.pos_Screen.y)  ;
                 //// khalil 
                  direction_two = 2;
                        con_two=1; }  
             ...
             if (event.key.keysym.sym==SDLK_c)   
                 {IMAGE_JOUEUR2.up=3;   posyi1 =IMAGE_JOUEUR2.pos_Screen.y ; 
                  saut_personnage2 ( &IMAGE_JOUEUR2,dt, IMAGE_JOUEUR2.pos_Screen.x,IMAGE_JOUEUR2.pos_Screen.y )  ; }  
...
/////////////
  if (event.key.keysym.sym == SDLK_t) {   
           indicate2 = 0 ;  IMAGE_JOUEUR2.d  =3; 
          }
      
          
          if (event.key.keysym.sym == SDLK_d) {
           indicate2 = 0 ; IMAGE_JOUEUR2.direction = 2 ;
            /// khalil 
               if (direction_two == 0) 
                {con_two=0; }
          }
           if (event.key.keysym.sym == SDLK_q) {
           indicate2 = 0 ;IMAGE_JOUEUR2.direction = 3 ;
           /// khalil  
            if (direction_two == 1) 
                {con_two=0; }
          }
         ...
         if (event.key.keysym.sym == SDLK_e) 
          ...
          if (event.key.keysym.sym==SDLK_x)  
                {indicates2=0 ; IMAGE_JOUEUR2.up=0;IMAGE_JOUEUR2.pos_Screen.y =pos1 ;
                 }

          
         if (event.key.keysym.sym==SDLK_c) 
              {  IMAGE_JOUEUR2.up=4;
                 if(IMAGE_JOUEUR2.direction==1 )  IMAGE_JOUEUR2.pos_Screen.x=IMAGE_JOUEUR2.pos_Screen.x+50  ;
                 if(IMAGE_JOUEUR2.direction==0 )  IMAGE_JOUEUR2.pos_Screen.x=IMAGE_JOUEUR2.pos_Screen.x-50  ;
                 IMAGE_JOUEUR2.pos_Screen.y=posyi1  ;  }  
...
if (con==1) 
{scrollingone(&B,direction_one,pas); }

if (con_two==1) 
{scrollingtwo(&B,direction_two,pas); }

animer_image(&A); 
animer_image_deux(&A2); 

///// badalt ihna zeda zidt parametre 1 joueru 1 wa 2 joueur 2  matinsech tabdil fi .h 
if (indicate ) 
{movePerso2(&IMAGE_main ,dt , 1 ) ;   }
else if  (indicate == 0 && IMAGE_main.direction == 2 )
{movePerso2(&IMAGE_main ,dt , 1 ) ;}
else if(indicate == 0 && IMAGE_main.direction == 3)
{movePerso2(&IMAGE_main ,dt , 1 ) ; }
else  {}

if (indicate2 ) 
{movePerso2(&IMAGE_JOUEUR2 ,dt , 2 ) ;   }
else if  (indicate2 == 0 && IMAGE_JOUEUR2.direction == 2 )
{movePerso2(&IMAGE_JOUEUR2 ,dt , 2 ) ;}
else if(indicate2 == 0 && IMAGE_JOUEUR2.direction == 3)
{movePerso2(&IMAGE_JOUEUR2 ,dt , 2 ) ; }
else  {}



 saut2( &IMAGE_main,dt,IMAGE_main.pos_Screen.y) ;
saut2( &IMAGE_JOUEUR2,dt,IMAGE_JOUEUR2.pos_Screen.y) ;
...
liberer_imageb2(IMAGE) ; 
liberer_imageb2(feu) ; 
liberer_imagep2(IMAGE_main)  ; 
liberer_imagep2(IMAGE_JOUEUR2)  ; 
liberer_imagep2(coeur)  ; 
liberer_imagep2(lost)  ;
...
{
//chargement de l'image
imgbtn->url="40.png" ; //pour la fonction d'initialisation de l'image du bouton on definit l'url de l'emplacement du fichier de l'image  
imgbtn->img=IMG_Load(imgbtn->url); 
if (imgbtn->img ==NULL)
   {printf("unable to load background image %s ",SDL_GetError());
   return ;}
...
{
//chargement de l'image
imgbtn->url="41.png" ; //pour la fonction d'initialisation de l'image du bouton on definit l'url de l'emplacement du fichier de l'image  
imgbtn->img=IMG_Load(imgbtn->url); 
if (imgbtn->img ==NULL)
   {printf("unable to load background image %s ",SDL_GetError());
   return ;}
...
{
//chargement de l'image
imgbtn->url="44.png" ; //pour la fonction d'initialisation de l'image du bouton on definit l'url de l'emplacement du fichier de l'image  
imgbtn->img=IMG_Load(imgbtn->url); 
if (imgbtn->img ==NULL)
   {printf("unable to load background image %s ",SDL_GetError());
   return ;}
...
{
//chargement de l'image
imgbtn->url="55.png" ; //pour la fonction d'initialisation de l'image du bouton on definit l'url de l'emplacement du fichier de l'image  
imgbtn->img=IMG_Load(imgbtn->url); 
if (imgbtn->img ==NULL)
   {printf("unable to load background image %s ",SDL_GetError());
   return ;}
...
{
//chargement de l'image
imgbtn->url="66.png" ; //pour la fonction d'initialisation de l'image du bouton on definit l'url de l'emplacement du fichier de l'image  
imgbtn->img=IMG_Load(imgbtn->url); 
if (imgbtn->img ==NULL)
   {printf("unable to load background image %s ",SDL_GetError());
   return ;}
...
{
//chargement de l'image
imgbtn->url="77.png" ; //pour la fonction d'initialisation de l'image du bouton on definit l'url de l'emplacement du fichier de l'image  
imgbtn->img=IMG_Load(imgbtn->url); 
if (imgbtn->img ==NULL)
   {printf("unable to load background image %s ",SDL_GetError());
   return ;}
...
{
//chargement de l'image
imgbtn->url="88.png" ; //pour la fonction d'initialisation de l'image du bouton on definit l'url de l'emplacement du fichier de l'image  
imgbtn->img=IMG_Load(imgbtn->url); 
if (imgbtn->img ==NULL)
   {printf("unable to load background image %s ",SDL_GetError());
   return ;}
...
//initialisation texte ,video et son!!!!
    ...
    int boucle =1,boucle1=0 ; //compteur sur la boucle de jeu
    int boucle2 =0  ; 
    int boucle3=0, boucle4=0, boucle5=0,boucle6=0,boucle7=0,boucle8=0,boucle9=0;

    int enter=0;
    float v=1;
	image1 S5, S6,S1,S7,S8,S9;
...
// initialisation des images son et texte
    initialiser_imageSTORY(&IMAGE) ;
    initialiser_imageSCENEE1(&IMAGE_BTN1) ;
    initialiser_imageSCENEE2(&IMAGE_BTN2) ;
    initialiser_imageSCENEE3(&IMAGE_BTN3) ;
    initialiser_ss5(&S5) ;
    initialiser_ss6(&S6) ;
    initialiser_ss7(&S7) ;
//---------------------------------------------boucle jeu
